$(function () {
  ...
  var getPictureCallback = function (d) {
    for (var i = 0; i < d.length; i++) {
      var authPicture = d[i].ProfilePic;
      var screenName = d[i].Handle;
      setPicture(authPicture, screenName);
    }
  }
  ...
  function otherAjax(callback) {
    var picture;
    $.ajax({
      url: 'http://dev.odimax.com/get_latest_tweets.php',
      success: function (data) {
        picture = data;
        callback(picture);
      }
  ...
  otherAjax(getPictureCallback);

  function setPicture(authPicture, screenName) {
    var pic = $("<div class='tweetImage' style='display: inline'><a href='https://twitter.com/intent/user?screen_name=" + screenName + "&action=see'><img src='" + authPicture + "'></a></div>").fadeIn("slow");
